...
void check_for_updates() {
    ...
    string new_version = ver.text;

    if (new_version.find(REDPOINT_VERSION) == string::npos) {
        cout << RED << "A new RedPoint version is available!" << endl;
        cout << BLUE << "Download it here: https://github.com/13-05/redpoint/blob/main/redpoint/versions/" << new_version << endl;
        Sleep(5000);
        exit(0);
    }
    else {
